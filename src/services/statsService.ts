...
class StatsService {
  ...
  async getUserStats(userId: string): Promise<any> {
    try {
      ...
      weekdayStats.rows.forEach((row: any) => {
  ...
  async getDailyProgress(userId: string, days = 30): Promise<any[]> {
    try {
  ...
  async getKanbanMetrics(userId: string): Promise<any[]> {
    try {
      const result = await pool.query(
        `SELECT 
  ...
  async getPriorityDistribution(userId: string): Promise<any[]> {
    try {
      const result = await pool.query(
        `SELECT
  ...
  async getCategoryCompletionTime(userId: string): Promise<any[]> {
    try {
      const result = await pool.query(
        `SELECT
  ...
  async getDashboardData(userId: string): Promise<any> {
    try {
      ...
      const completedLastWeek = await pool.query(
        `SELECT COUNT(*) AS count
        FROM tasks
        WHERE 
          user_id = $1 
